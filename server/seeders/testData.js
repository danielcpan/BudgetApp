...
const createTestData = async () => {
  ...
  let eatingOutCategory = await factory.create('Category', { name: 'Eating Out', icon: 'fa-utensils', color: "#5ad09a", userId: user.id })
  let rentCategory = await factory.create('Category', { name: 'Rent', icon: 'fa-home', color: "#ff0000", userId: user.id })
  let gasCategory = await factory.create('Category', { name: 'Gas', icon: 'fa-gas-pump', color: "#4684FF", userId: user.id })
